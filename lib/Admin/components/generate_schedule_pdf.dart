...
Future<void> generateSchedulePDF(List<Map<String, dynamic>> data) async {
  ...
  final fileName = "${formattedMonth}_SCHEDULE_REPORT.pdf";
    ...
    pw.MultiPage(
      ...
      build: (pw.Context context) => [
        pw.Table.fromTextArray(
          ...
          headers: ['TeacherId', 'Subject', 'Grade', 'Class Day', 'Start Time', 'End Time', 'Class Fee'],
          data: data
              .map((item) => [
                    item['TeacherId']?.toString() ?? '',
                    item['Subject']?.toString() ?? '',
                    item['Grade']?.toString() ?? '',
                    item['ClassDay']?.toString() ?? '',
                    item['StartTime']?.toString() ?? '',
                    item['EndTime']?.toString() ?? '',
                    item['ClassFee']?.toString() ?? '',
                  ])
              .toList(),
        ),
